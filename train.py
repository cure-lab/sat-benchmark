#!/usr/bin/env python3
""" End-to-End SAT solver Training Script

This is intended to be a lean and easily modifiable End-to-End SAT solver training script that reproduces SAT
...
Modified by Lee Man to support SAT Benchmarking (https://github.com/lee-man)
NOTE: I comment the lines we do not need for now (instead of deleting them directly) and will check whether we should add those functionalities later.
"""
import argparse
import logging
import os
...
# satb
from satb import utils
from satb.data import create_dataset, create_loader #, resolve_data_config, Mixup, FastCollateMixup, AugMixDataset
from satb.loss import JsdCrossEntropy, SoftTargetCrossEntropy, BinaryCrossEntropy, \
    LabelSmoothingCrossEntropy, SmoothStep
from torch.nn import L1Loss, SmoothL1Loss, MSELoss, BCELoss, BCEWithLogitsLoss, CrossEntropyLoss
from satb.models import create_model, safe_model_name, resume_checkpoint, \
    model_parameters
...
parser = argparse.ArgumentParser(description='PyTorch SAT Solver Training')
...
group.add_argument('--bce-loss', action='store_true', default=False,
                    help='Enable BCE loss for NeuroSAT.')
group.add_argument('--smooth-step-loss', action='store_true', default=False,
                    help='Enable SmoothStep loss for DGDAGRNN.')
group.add_argument('--l1-loss', action='store_true', default=False,
                    help='Enable l1 loss for DeepSAT.')                   
...
def main():
    ...
    # FIXME: enable gradient checking in SAT models.
    ...
    # mli: NOTE: may add this part later.
    ...
    if args.bce_loss:
        ...
        validate_loss_fn = BCEWithLogitsLoss()
    elif args.smooth_step_loss:
        train_loss_fn = SmoothStep()
        validate_loss_fn = SmoothStep()
    elif args.l1_loss:
        train_loss_fn = L1Loss()
        validate_loss_fn = L1Loss()
    else:
